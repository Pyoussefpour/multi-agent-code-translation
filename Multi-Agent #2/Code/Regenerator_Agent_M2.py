...
def Check_error_type(state:AgentState):
    ...
    if response.content.upper() == "FUNCTION":
        input_code_analysis = state["input_code_analysis"]
        extra_information = ""
    
    else:
        input_code_analysis = state["UnitTest_script"]
        extra_information = "The error is caused by the unit test script, do not change the f_filled function, as it is assumed to be correct"
    return {"Regen_return_node": response.content, 
            "input_code_analysis": input_code_analysis, 
            "extra_info_analysis": extra_information,
...
- and an analysis of the error to help you with fixing the error
...
```
    """,
    input_variables=["input_language", "target_language", "input_code", "code_status", "translated_code", "error_analysis"])
...
def Regenerate_Function_node(state: AgentState):
    ...
    messages = [
        SystemMessage(content=Regenerate_Function_Prompt.format(input_language=state['input_lang'], 
                                                                ...
                                                                error_analysis=state['Regenerator_analysis']))
    ]
    response = model.invoke(messages)
...
```
    """,
    input_variables=["target_language", "code_status", "UnitTest_script", "error_analysis"]
)  
