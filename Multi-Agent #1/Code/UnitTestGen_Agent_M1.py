...
```
    """, input_variables = ["input_language", "input_code", "output_language", "example_input"])
...
def Create_Input_node(state: AgentState): 
    ...
    return {"sample_inputs": response.content, "UnitTest_status": "STARTED", "UnitTest_history": UnitTest_history}


Create_TestCode_Prompt = PromptTemplate(
    template="""
You are a software engineer tasked with generating a standalone {input_language} script that logs function outputs for a given list of inputs.

### Objective
Given:
- A function in {input_language} to be tested
- A list of input arguments in **JSON format** (not a file)

Produce a standalone program in {input_language} that:
1. **Includes** all necessary imports and boilerplate (e.g., class/method declarations if required)
2. **Incorporates** the provided code exactly as-is — do not modify it
3. **Implements a main routine** that:
   - Iterates through the input argument sets
   - Calls the correct function with those inputs
   - Collects the outputs (or error if an exception occurs)
   - Stores the input and output pairs in a list
4. **Writes the result to a file** named `results.json` located in the specified directory: `{directory}`
   - Each item in the JSON array should be of the form:  
     `{{ "input": [...], "output": <value> }}`
   - The file should be written to the ABSOLUTE path: {directory}
   - Use the appropriate file path creation for the target language
{additional_instruction}

### Requirements
- The script must be fully executable and self-contained
- The function should be called with the input args using correct unpacking
- Catch any runtime errors to avoid crashing; store `"error"` in the output if an exception occurs
- Use only safe and standard libraries
- The output file must be name `results.json`
- Output file must be saved to {directory}
- Do not alter the original function code
- Do not print anything to the console
- Output only the complete, correct code — no explanation, comments, or formatting outside the fenced code block
- When writing to the file, use the complete absolute path rather than a relative path
...
Script
```
    """, input_variables = ["input_language", "input_code", "sample_inputs", "additional_instruction", "directory"])
...
if __name__ == "__main__":
    ...
    return {"test_code": response.content, "JSON_dir": current_dir, "UnitTest_history": state['UnitTest_history'] + UnitTest_history}
...
def Check_TestCode_node(state: AgentState):
    ...
    return {"code_state": response.content, "UnitTest_history": state['UnitTest_history'] + UnitTest_history}
...
```
""",
input_variables=["input_language", "code_state", "test_code"])
...
def Run_TestGen_node(state: AgentState):
    ...
    if not error:
        ...
        return {"code_state": error, "UnitTest_history": state['UnitTest_history'] + UnitTest_history}
