...
def create_test_file(file_path: str, translated_code: str, output_language: str):
    ...
    if output_language.upper() == "PYTHON":
        ...
        mvn_path = os.path.join(current_dir, "Eval")
...
def write_result(input_file_name: str, translated_code: str, input_language: str, output_language: str, result: str, passed: bool, n_iterations: int, max_iter_reached: bool, output_file: str, Translator_history: str):
    """
    ...
    with open(output_file, 'a') as file:
        ...
        if not file_exists or os.path.getsize(output_file) == 0:
            writer.writerow(['Input File', 'Input Language', 'Output Language', 'Result', 'Passed All Tests', 'Number of Iterations','Max Iter Reached', 'Translated Code', 'History'])

        writer.writerow([input_file_name, input_language, output_language, result, passed, n_iterations, max_iter_reached, translated_code, Translator_history])

    


def eval_pipeline(file_path: str, translated_code: str, input_language: str, output_language: str, num_iterations: int, max_iter_reached: bool, Translator_history: str):
    """
    ...
    output_file = os.path.join(os.getcwd(), f"Bonus_results_{input_language}_{output_language}.csv")
    file_name = os.path.basename(file_path)
    print("result_string: ", result_string)
    write_result(file_name.split(".")[0], translated_code, input_language, output_language, 
                 result_string, passed, num_iterations, max_iter_reached, output_file, Translator_history)
